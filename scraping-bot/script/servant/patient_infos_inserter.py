...
def serve(db):
    ...
    if not is_exist_on_db:
        ...
        discharge = sheet.cell(row=sheet.max_row, column=10).value - sheet.cell(row=sheet.max_row-1, column=10).value
        death = sheet.cell(row=sheet.max_row, column=9).value - sheet.cell(row=sheet.max_row - 1, column=9).value
        sickbed_total = 509
