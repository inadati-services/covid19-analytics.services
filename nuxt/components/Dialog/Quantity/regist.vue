<template>
    <div>
        <v-dialog v-model="visible" width="500" @click:outside="close">
            <v-card>
                <v-card-title class="headline">新規データ追加</v-card-title>
                <v-divider></v-divider>
...
import qidp from "~/servant/QuantityInitialDataProvider";
...
export default {
    ...
    data() {
        return {
            dialogData: DataCloner.serve(qidp.serve(this.pref))
        };
    },
    methods: {
        close() {
        ...
        async regist() {
            await Promise.all([
                this.$rapi.Mutation.registQuantity(this.pref, this.dialogData)
            ]);
            this.close();
        }
    },
    ...
    watch: {
        visible(newValue, oldValue) {
            if(newValue == true){
                this.dialogData = DataCloner.serve(qidp.serve(this.pref))
            }
        }
    }
};
